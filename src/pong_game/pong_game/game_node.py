...
from pong_interfaces.srv import Reset
...
class PongGame(Node):
    def __init__(self):
        ...
        # Set environment variables for pygame before initialization
        ...
        self.screen = pygame.display.set_mode(
            ...
            pygame.SWSURFACE  # Use software surface, not OpenGL
        )
        ...
        # Log initialization
        self.get_logger().info("Pong game initialized")
        self.get_logger().debug(f"Game screen size: {self.screen_width}x{self.screen_height}")
    ...
    def reset_game(self):
        ...
        self.get_logger().debug("Resetting game state")
    ...
    def player1_callback(self, msg):
        ...
        self.get_logger().debug(f"Player 1 position updated: {self.player1_pos}")

    def player2_callback(self, msg):
        self.player2_pos = int(msg.y_position)
        self.get_logger().debug(f"Player 2 position updated: {self.player2_pos}")
    ...
    def run_frame(self):
        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                self.get_logger().info("Quit event received")
                return False
        ...
        if self.game_active:
            ...
            if self.ball_pos[0] < 0:
                ...
                self.reset_ball()
                self.get_logger().info(f"Player 2 scored! Score: {self.player1_score} - {self.player2_score}")

            if self.ball_pos[0] > self.screen_width:
                self.player1_score += 1
                self.reset_ball()
                self.get_logger().info(f"Player 1 scored! Score: {self.player1_score} - {self.player2_score}")

            # Check for game over
            if self.player1_score >= self.get_parameter('max_games').value or \
                    self.player2_score >= self.get_parameter('max_games').value:
    ...
    def reset_ball(self):
        if self.player1_score < self.get_parameter('max_games').value and \
                self.player2_score < self.get_parameter('max_games').value:
            self.ball_pos = [self.screen_width // 2, self.screen_height // 2]
            self.ball_speed = [
                random.choice([-1, 1]) * self.get_parameter('ball_speed_x').value,
...
def main(args=None):
    ...
    try:
        ...
        while rclpy.ok() and running:
            ...
            rclpy.spin_once(pong_game, timeout_sec=0.001)

            # Run one game frame
            running = pong_game.run_frame()
